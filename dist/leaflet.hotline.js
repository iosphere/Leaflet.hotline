 ...
 */
var Hotline = function Hotline(canvas, map) {
...
Hotline.prototype = {
    ...
    zoomWeight: function zoomWeight(_zoomWeight) {
        this._zoomWeight = _zoomWeight;
        return this;
    },
     ...
     */
    outlines: function outlines(_outlines) {
        this._outlines = _outlines;
        return this;
    },
     ...
     */
    palette: function palette(_palette) {
        this._palette = this.calcPalette(_palette);
        return this;
    },

    calcPalette: function calcPalette(palette) {
    ...
    unknownColor: function unknownColor(_unknownColor) {
        var palette = this.calcPalette([_unknownColor]);
        this._unknownColor = [palette[0], palette[1], palette[2]];
        return this;
    },

    maxGradient: function maxGradient(_maxGradient) {
        this._maxGradient = _maxGradient;
        return this;
    },

    minGradient: function minGradient(_minGradient) {
        this._minGradient = _minGradient;
        return this;
    },

    minColorLength: function minColorLength(_minColorLength) {
        this._minColorLength = _minColorLength;
        return this;
    },

    gradientPercent: function gradientPercent(_gradientPercent) {
        this._gradientPercent = _gradientPercent;
        return this;
    },

    color: function color(_color) {
        var palette = this.calcPalette([_color]);
        this._color = [palette[0], palette[1], palette[2]];
    },
    ...
    data: function data(_data) {
        ...
        for (i = 0, dataLength = _data.length; i < dataLength; i++) {
            path = _data[i];
            preCalculatedData[i] = [];
    ...
    setGradientPoint: function setGradientPoint(path) {
    ...
    setRGBData: function setRGBData(data) {
            ...
            for (j = 0, pathLength = path.length; j < pathLength; j++) {
                ...
                if (!pointCurrent.rgb) {
                    if (pointPrev && pointNext && pointPrev.rgb && pointNext.rgb) {
                        pointCurrent.rgb = [Math.floor((pointPrev.rgb[0] + pointNext.rgb[0]) / 2), Math.floor((pointPrev.rgb[1] + pointNext.rgb[1]) / 2), Math.floor((pointPrev.rgb[2] + pointNext.rgb[2]) / 2)];
                    } else {
                        pointCurrent.rgb = this.getRBGForPoint(pointCurrent);
                    }
                }
    ...
    getRBGForPoint: function getRBGForPoint(point) {
        var rgb = point.rgb;
        if (!rgb) {
     ...
     */
    clear: function clear(clearData) {
        if (clearData) {
            this._data = [];
        }
     ...
     */
    getRGBForValue: function getRGBForValue(value) {
    ...
    _getActualWeight: function _getActualWeight(obj) {
        obj = obj || this;
    ...
    _drawOutlines: function _drawOutlines(ctx, clear) {
        if (!this._outlines) {
            return;
        }
     ...
     * @private
     */
    _drawOutline: function _drawOutline(ctx, clear, color, width) {
     ...
     * @private
     */
    _drawHotline: function _drawHotline(ctx) {
...
L.HotlineRenderer = L.Canvas.extend({
    ...
    _updateOptions: function _updateOptions(layer) {
...
function makeGradienter() {
    var MathUtil = {
         ...
         * @returns {*}
         */
        findPointOnLine: function findPointOnLine(point1, point2, distance) {
            if (distance === 0) {
                return point1;
            }
        ...
        distanceTo: function distanceTo(point1, point2) {
